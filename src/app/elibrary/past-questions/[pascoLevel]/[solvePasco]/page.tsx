...
export default function ReadFile({ params }:{ params: {solvePasco: string}}){

    const fileIndex = parseInt(params.solvePasco) - 1;

    const getPascoFiles = courses.map((value) => value.pasco)
    const files = getPascoFiles[0];
    const readMoreFiles = files.filter((file) => file.id !== parseInt(params.solvePasco));
